...
if ($PSVersionTable.PSVersion.Major -lt 6) {
	#Note: this is a built-in var in pwsh 6+
	[bool] $IsLinux = $false
}
...
Get-ChildItem $ScriptPath/public -Recurse -Filter "*.ps1" -File | ForEach-Object {
	...
	([System.Management.Automation.Language.Parser]::ParseInput((Get-Content -Path $_.FullName -Raw), [ref] $null, [ref] $null)).FindAll( { $args[0] -is [System.Management.Automation.Language.FunctionDefinitionAst] }, $false) | ForEach-Object {
		if (($IsLinux) -and ($_.Name -match '-SID$')) {
			#Windows Security Functions are not available in Linux
		} else {
...
#region Load Formats
if (test-path $ScriptPath\formats\brshConversions.format.ps1xml) {
	Update-FormatData $ScriptPath\formats\brshConversions.format.ps1xml
}
#endregion Load Formats

if (-not $Quiet) {
	Get-brshConversionsHelp
}

#region Module Cleanup
$ExecutionContext.SessionState.Module.OnRemove = {
	# cleanup when unloading module (if any)
	Get-ChildItem alias: | Where-Object { $_.Source -match "brshConversions" } | Remove-Item
	Get-ChildItem function: | Where-Object { $_.Source -match "brshConversions" } | Remove-Item
	Get-ChildItem variable: | Where-Object { $_.Source -match "brshConversions" } | Remove-Item
}
